 ...
 */
void control_valve(boolean state){
  if(state == VALVE_OPEN){
  ...
  */
  ...
  if(valve_is_closed && sampling && 
    ...
    *(unsigned long *)(lorate_data + 1) = now_ms;
    // ambient pressure sensor and liquid sensor have same i2c address.
  ...
  if(Serial.available()){
	...
	if((byte)command[0] == (byte)cksum){ 
	  ...
	  */
	  ...
	  control_pump(command[3]);
	  control_valve(command[4]);
